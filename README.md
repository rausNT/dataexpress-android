...
- диагностический журнал можно скопировать в буфер, сохранить как `.jsonl` или отправить вручную через системное меню Android; после аварийного завершения меню отправки открывается автоматически;
- если Wine запустился, но окно DataExpress не появилось за 20 секунд, приложение предлагает повторить запуск в совместимом режиме: без `esync` и с фиксированным экраном 1280×800;
- журнал фиксирует появление X11-окон и реальный запас Java heap, поэтому код 137 можно отличить от ручного закрытия и OOM;
...
4. Развернуть серверный приёмник автоматической диагностики; до этого использовать встроенный ручной экспорт `.jsonl`.
