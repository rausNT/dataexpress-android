...
"""
import hashlib,json,re,subprocess,sys,time,traceback,xml.etree.ElementTree as ET,zipfile
...
def node(rid=None,text=None,desc=None,contains=None,enabled=None):
 ...
 return None
def swipe(down=True):
 w,h=map(int,re.findall(r'(\d+)x(\d+)',shell('wm','size'))[-1]);shell('input','swipe',str(w//2),str(int(h*(.77 if down else .30))),str(w//2),str(int(h*(.30 if down else .77))),'240');time.sleep(.25)
def find(**kw):
 n=node(**kw)
...
def tap(n):
 x1,y1,x2,y2=map(int,re.findall(r'\d+',n.get('bounds','')));shell('input','tap',str((x1+x2)//2),str((y1+y2)//2));time.sleep(.35)
def click(**kw):tap(find(**kw))
def present(**kw):
 n=find(**kw);assert n is not None;return n
def top():
 for _ in range(4):swipe(False)
def screenshot(name):
...
def permission(allow):
 choices=('permission_allow_foreground_only_button','permission_allow_button','permission_allow_one_time_button') if allow else ('permission_deny_button',)
 for _ in range(5):
  for n in dump().iter('node'):
   if any(n.get('resource-id','').endswith('/'+s) for s in choices):tap(n);return
  time.sleep(.5)
...
def fill_word(word):
 for ch in word:
  matches=[n for n in dump().iter('node') if n.get('content-desc','').startswith('Буква '+ch+',') and n.get('enabled')=='true']
  if not matches:top();matches=[n for n in dump().iter('node') if n.get('content-desc','').startswith('Буква '+ch+',') and n.get('enabled')=='true']
  assert matches,'Missing letter '+ch;tap(matches[0])
def audio_logs():return adb('logcat','-d','-s','BukvarAudio:I','*:S').decode(errors='replace')
def play_and_wait(rid):
 before=audio_logs().count('PLAYBACK_OK');click(rid=rid)
 deadline=time.monotonic()+20
 while time.monotonic()<deadline:
  if audio_logs().count('PLAYBACK_OK')>before:return
  time.sleep(.5)
 raise AssertionError('MediaPlayer did not report completion within 20 seconds')
try:
 blob=APK.read_bytes()
 ...
 installation=adb('install',str(APK)).decode();assert 'Success' in installation;ok('Install release APK',installation.strip());(OUT/'install.txt').write_text(installation)
 ...
 play_and_wait('play');ok('Native recording playback completes');top();screenshot('04-recorded')
 click(rid='record_model');time.sleep(2);click(rid='record_model');play_and_wait('play_model');ok('Adult audio sample records and plays')
 ...
 shell('settings','put','system','user_rotation','0');time.sleep(2);click(rid='next');present(rid='prompt',text='мама');top();screenshot('09-reading');ok('Reading word page')
 click(rid='split');present(rid='prompt',text='ма · ма');top();screenshot('10-syllables');ok('Syllable assistance toggles');click(rid='split');click(rid='speech')
 ...
 fatal=[b for b in logs.split('FATAL EXCEPTION:')[1:] if PKG in b[:1500]];assert not fatal,'Application crashed: '+str(fatal);ok('No app FATAL EXCEPTION in device log')
 (OUT/'checks.json').write_text(json.dumps(CHECKS,ensure_ascii=False,indent=2));print('ALL '+str(len(CHECKS))+' DEVICE CHECKS PASSED',flush=True)
except Exception as exc:
 CHECKS.append({'test':'failure','status':'failed','detail':str(exc)});(OUT/'checks.json').write_text(json.dumps(CHECKS,ensure_ascii=False,indent=2));(OUT/'failure.txt').write_text(traceback.format_exc())
